...
?>
	...
	<script>
		...
		var lineChartData = {
			labels :[
		<?php
		$sql ="SELECT fechaEntrevista FROM entrevistabp Where year(fechaEntrevista)=year(Now()) and co = '$co' group by fechaEntrevista";
			...
			datasets : [
				{
					...
					$sql ="SELECT fechaEntrevista, co, count(*) as conteo FROM entrevistabp  Where year(fechaEntrevista)=year(Now()) and co = '$co' group by fechaEntrevista";
				$resultado = mysqli_query($objCnx,$sql); 
