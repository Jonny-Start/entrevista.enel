<?php
	...
	function getValor($campo, $campoWhere, $valor)
	{
		...
		$stmt = $objCnx->prepare("SELECT $campo FROM usuario WHERE $campoWhere = ? LIMIT 1");
	...
	function generaTokenPass($co)
	{
		global $objCnx;
		
		$token = generateToken();
		
		$stmt = $objCnx->prepare("UPDATE usuario SET token_password=?, password_request=1 WHERE co = ?");
		$stmt->bind_param('ss', $token, $co);
		$stmt->execute();
		$stmt->close();
	...
	function enviarEmail($email, $asunto, $cuerpo){
		...
		$mail->isSMTP();
		$mail->SMTPAuth = true;
		$mail->SMTPSecure = 'tls'; //Modificar- tls es para gmail 
		$mail->Host = 'smtp.gmail.com'; //Modificar- Host de gmail 
		$mail->Port = 587; //Modificar-puerto de gmail
		
		$mail->Username = 'jonnyalejandro.ca0910@gmail.com'; //Modificar
		$mail->Password = '1006844604jonny1123321772madre'; //Modificar
		
		$mail->setFrom('jonnyalejandro.ca0910@gmail.com', 'Jonny Alejandro'); //Modificar
	...
	function emailExiste($email)
	{
		...
		$stmt = $objCnx->prepare("SELECT co FROM usuario WHERE correo_electronico = ? LIMIT 1");
